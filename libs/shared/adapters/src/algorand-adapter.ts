...
export default class AlgorandAdapter {
  ...
  constructor(
    options: AlgorandAdapterOptions,
    logger: pino.Logger<unknown>,
    appSecret: string
  ) {
  ...
  generateAccount(passphrase: string): PublicAccount {
    ...
    const encryptedMnemonic = encrypt(mnemonic, passphrase, this.appSecret)
    return {
  ...
  async initialFundTransactions(
    ...
    const account = algosdk.mnemonicToSecretKey(
      decrypt(encryptedMnemonic, passphrase, this.appSecret)
    )
  ...
  isValidPassphrase(encryptedMnemonic: string, passphrase: string) {
    try {
      const mnemonic = decrypt(encryptedMnemonic, passphrase, this.appSecret)
      if (mnemonic) {
  ...
  async closeCreatorAccount(creator: PublicAccount, creatorPassphrase: string) {
    const account = algosdk.mnemonicToSecretKey(
      decrypt(creator.encryptedMnemonic, creatorPassphrase, this.appSecret)
    )
  ...
  async getCreatorAccount(initialBalance: number, creatorPassphrase: string) {
    ...
    const creator = await this.createAccount(creatorPassphrase, initialBalance)
  ...
  async generateClawbackTransactions(options: {
    ...
    const toAccount = algosdk.mnemonicToSecretKey(
      decrypt(options.encryptedMnemonic, options.passphrase, this.appSecret)
    )
